...
- **Temporary HP:** a separate hit-point buffer (from `false life`, `heroism`, a paladin's lay-on-hands variants, etc.) that absorbs damage **before** real HP, is **not** restored by healing, doesn't stack (take the higher of old/new), and is cleared on a long rest. Add a `temp_hp` field on `Character`/`NPC`, drain it first in `apply_damage`, and have `heal` ignore it. Pairs with the utility/healing-spell work above.
