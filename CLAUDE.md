...
Already implemented: combat, the death-save cycle, social (`influence_npc`), companions (`recruit_npc` — cross-scene allies that fight for the party), stealth (`attempt_ambush`), scene/gate/loot, spells with a capped slot economy (`max_spell_slots`), `use_item` (self or administered to a downed ally), `skill_check`, and `add_npc`. For what's next, see the README's "Need to implement" (general ability saving throws, hazards/traps) and "Potential implementations" (ranked) sections, plus the two-model split in DECISIONS.md. Note: a general ability/skill *check* tool already exists (`skill_check`); the missing twin is the *saving throw* (resisting an effect), distinct from the death-save cycle.
