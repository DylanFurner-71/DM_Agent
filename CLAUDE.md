...
Already implemented: combat, the death-save cycle, social (`influence_npc`), companions (`recruit_npc` — cross-scene allies that fight for the party), stealth (`attempt_ambush`), scene/gate/loot, spells with a capped slot economy (`max_spell_slots`), `use_item` (self or administered to a downed ally), `skill_check`, `saving_throw` (the reactive twin of `skill_check` — resisting an effect; not turn-guarded, adds proficiency for `save_proficiencies`), and `add_npc`. For what's next, see the README's "Need to implement" (hazards/traps, /undo + autosave, scenario validator) and "Potential implementations" (ranked) sections, plus the two-model split in DECISIONS.md. Note: the proactive `skill_check` and its reactive twin `saving_throw` both now exist; the next gap is hazards/traps, which combine a save/check with `apply_dice` damage.
