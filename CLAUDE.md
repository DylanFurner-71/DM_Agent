...
python3 -m pytest -q                                  # enforcement tests — no API needed
python3 -m src.main                                   # default scenario (data/scenario.json)
python3 -m src.main data/my_scenario.json             # custom scenario
python3 -m src.main savegame.json                     # resume a saved game
python3 -m src.main --seed 42                          # fix the dice RNG for reproducible rolls
python3 -m src.main --help                             # show all options (--seed, --no-hud, --plain)
python3 -m src.validate data/scenario.json            # lint a scenario before play (no API)
```

In-session commands: `/help`, `/state`, `/hud`, `/recap`, `/roll <notation>`, `/undo`, `/trace`, `/full_trace`, `/cost`, `/export [path]`, `/save [path]`, `/quit`. The game autosaves to `saves/autosave.json` after every turn (`python3 -m src.main saves/autosave.json` to resume).
