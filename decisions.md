...
## 8. Lean on parallel tool_use to cut tool-loop hops

**Decision.** Actively prompt the model to batch *independent* tool calls into a single
response (parallel `tool_use` blocks) rather than resolving a multi-tool turn one tool per
hop. This is a **prompt-only** change: the tool loop in `dm_agent.py::_execute` already
iterates every `tool_use` block in a response and dispatches each via `tools.dispatch()`
**in array order**, accumulating all `tool_result`s before the next hop — so the mechanism
predates this decision; what changed is the system prompt now tells the model to use it by
default (the `PARALLELIZE INDEPENDENT TOOL CALLS` block in `RESOLVING AN ACTION`), with an
explicit batch-these / keep-these-sequential split.

**Why.** Profiling (the per-call stats sidecars: 40 demo runs, 305 turns) reaffirmed the
~3.3s fixed cost *per API call* and ~2.43 calls/turn — so calls-per-turn, not output size,
is the lever (the same finding behind Decisions 5 and 6). A multi-tool turn resolved one
tool per hop pays a full round-trip for each extra hop; ~95/305 turns already batched, so
the win is in pushing the rest (loot sweeps, a `take_item` plus the `set_quest_flag` that
records the find) onto the batched path. Listed in the README's "remaining latency levers"
as Low–medium payoff, low code.

**Trade-off — prompt-level and unreliable.** This is a soft instruction: the model may
ignore it (no batch — we simply pay the old hop count, no worse than before) or, worse,
batch a *dependent* pair. **Why that's safe anyway:** the engine is the backstop, per the
shared principle — it owns the numbers, so how calls are *grouped* cannot corrupt state.
The one genuinely unsafe pairing, `start_combat` + a same-turn offensive, is already
barriered: `start_combat` sets `combat_starting`, and `attack`/`cast_spell` refuse with
reason `combat_starting` (without consuming the action), so the attack is *rejected*, not
mis-applied — proven by `tests/test_agent.py::test_take_turn_start_combat_defers_attack`.
Turn guards (which reset `action_used` on a rejected action) cover the in-combat case.
And because blocks dispatch in array order with state mutating between them, a *correctly
ordered* dependent batch still resolves correctly. So a wrong batch costs at most a retry
(`ok=false` the model narrates around), never a wrong number.

**Scope — what's unaffected.** Pure prompt/presentation. No change to dispatch, the loop,
enforcement, turn order, or redaction. Combat turns are largely unaffected (the player gets
one action; the combat fast-path already breaks on `action_used` or a hard-stop `ok=false`);
the gains land out of combat, on exploration/loot/flag turns.

**Guardrails.** A no-API test asserts three independent tools (two `take_item` + a
`set_quest_flag`) emitted in one response resolve in a **single** tool-use hop with all
results `ok`; `test_take_turn_start_combat_defers_attack` stays as the unsafe-batch
guardrail (the dependent call is refused).

**Reversibility.** Entirely localized to the `RESOLVING AN ACTION` block of `SYSTEM_PROMPT`
— revert that paragraph and the model falls back to one-tool-per-hop with no other change.
Low lock-in.
